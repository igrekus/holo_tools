...
//   static double double_amp[KINO_SIZE_X][KINO_SIZE_Y];
   ...
   char input_file_name[50]="in.arx";                  // Имя входного файла с изображением
...
//------------------ Считываем файл leptonica-ой ----------------------------
   ...
   output_re_file_name=strtok(tmpstr, ".");
   tmpstr=strdup(input_file_name);
   output_im_file_name=strtok(tmpstr, ".");
   tmpstr=strdup(input_file_name);
   output_bin_file_name=strtok(tmpstr, ".");
